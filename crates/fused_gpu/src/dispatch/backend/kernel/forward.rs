...
#[inline]
pub fn lower_forward<'a>(
    graph: &'a Graph<'a>,
    meta: Metadata,
    saved: &[SaveIndicator],
    options: &CompilationOptions,
) -> Result<KernelsChained<'a>, Error> {
    let mut kernels: Vec<Dependencies<LinkedKernel<'a>>> = Vec::new();
...
fn eval_node<'a>(
    ...
    match node.op {
        ...
        GraphOp::ConstU32(value) => {
            kernel.raw.overwrite_var(out, Op::ConstU32 { value });
        }

        GraphOp::ConstI32(value) => {
            kernel.raw.overwrite_var(out, Op::ConstI32 { value });
        }
