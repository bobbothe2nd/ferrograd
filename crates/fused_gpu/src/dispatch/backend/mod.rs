...
pub enum GraphOp<'a> {
    ...
    Custom {
        ...
        save: fn(NodeId, &Node<'_>, &Graph<'_>, &mut [SaveIndicator]),
        valid_shape: fn(NodeId, &Node<'a>, &Graph<'a>, &mut Vec<Error<GraphErrorContext<'a>>>),
        arity: u8,
        need_dims: bool,
        stable_iter: bool,
        auto_save: bool,
...
fn check_shapes<'a>(graph: &Graph<'a>, errors: &mut Vec<Error<GraphErrorContext<'a>>>) {
    for (node_id, node) in graph.nodes.iter().enumerate() {
...
impl<'a> Graph<'a> {
    ...
    pub fn get_adjacent(&self, node: NodeId, user: NodeId) -> Vec<NodeId> {
        ...
        for input in user_inputs {
            ...
            let outputs = &self.nodes[*input].outputs;

            adjacent.reserve(outputs.len());

            for output in outputs {
                adjacent.push(*output);
            }
        }
